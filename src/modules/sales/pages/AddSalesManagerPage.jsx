          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} className={`${inputClass} pr-10`} placeholder="Enter New Password" value={form.password} onChange={(e) => setForm((prev) => ({ ...prev, password: e.target.value }))} onBlur={() => setTouched(true)} />
              ...
              {touched && errors.password && <p className="mt-1 text-xs text-[var(--admin-danger-text)]">{errors.password}</p>}
            </div>
            <div>
              <label className="admin-text mb-2 block text-sm font-semibold">Confirm Password</label>
              <div className="relative">
                <input type={showConfirm ? "text" : "password"} className={`${inputClass} pr-10`} placeholder="Confirm New Password" value={form.confirmPassword} onChange={(e) => setForm((prev) => ({ ...prev, confirmPassword: e.target.value }))} onBlur={() => setTouched(true)} />
              ...
              {touched && errors.confirmPassword && <p className="mt-1 text-xs text-[var(--admin-danger-text)]">{errors.confirmPassword}</p>}
            </div>
          </div>

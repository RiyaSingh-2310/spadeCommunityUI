...
const LIST_PATH = "/user-screening/create-survey";
...
function CreateSurveyFormPage({ isDarkMode, mode = "add" }) {
  ...
  const removeQuestionAtIndex = (index) => {
    const removed = form.questions[index];
    if (removed?.recordId) {
    ...
    try {
      if (isEdit) {
        for (const recordId of deletedRecordIds) {
          await deleteScreeningQuestion(recordId);
        }
        ...
        const successMessage = lastData?.message || "Survey updated.";
        navigate(LIST_PATH, {
          replace: true,
          state: {
            flash: {
      ...
      const successMessage = lastData?.message || "Survey created.";
      navigate(LIST_PATH, {
        replace: true,
        state: {
          flash: {
  ...
  if (isEdit && loadFailed) {
    return (
      <div className="space-y-6">
        ...
        <p className="admin-text-muted text-sm">Survey not found.</p>
        <button
          type="button"
  ...
  const pageTitle = isEdit ? "Edit Survey" : "Add Survey";
